...
if __name__ == "__main__":
    ...
    # Rename chormosomes in the perfect file output (new)
    var1 = "cat %s | awk 'BEGIN{FS=OFS=\"\\t\"}" % outfile_perfect
    var2 = " (/^@/ && !/@SQ/){print $0}"
    var3 = " $2~/^SN:[1-9]|^SN:X|^SN:Y|^SN:MT/{print $0} "
    var4 = " $3~/^[1-9]|X|Y|MT/{$3=\"chr\"$3; print $0} '"
    var5 = " | sed 's/SN:/SN:chr/g' | sed 's/chrMT/chrM/g' > "
    name_change_temp = directory + "/namechanged_perfect_temp.sam"
    subprocess.call(var1 + var2 + var3 + var4 + var5 + name_change_temp, shell=True)
    subprocess.call(["mv", name_change_temp, outfile_perfect])
